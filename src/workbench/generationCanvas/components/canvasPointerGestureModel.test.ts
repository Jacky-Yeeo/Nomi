...
describe('generation canvas pointer arbitration', () => {
  it('gives blank primary drag to marquee without a modifier', () => {
    expect(
      resolveCanvasPointerDownAction({
        button: 0,
        spaceHeld: false,
  ...
  it('keeps Shift + blank primary drag on the same marquee path', () => {
    expect(
      resolveCanvasPointerDownAction({
        button: 0,
        spaceHeld: false,
  ...
  it('models the complete chord lifecycle while the primary pointer stays down', () => {
    expect(isCanvasPanButtonHeld(2, { buttons: 3, spaceHeld: false })).toBe(true)
    expect(isCanvasPanButtonHeld(2, { buttons: 1, spaceHeld: false })).toBe(false)
    expect(isCanvasPanButtonHeld(1, { buttons: 5, spaceHeld: false })).toBe(true)
    expect(isCanvasPanButtonHeld(1, { buttons: 1, spaceHeld: false })).toBe(false)
    expect(isCanvasPanButtonHeld(0, { buttons: 1, spaceHeld: true })).toBe(true)
    expect(isCanvasPanButtonHeld(0, { buttons: 1, spaceHeld: false })).toBe(false)
  })
