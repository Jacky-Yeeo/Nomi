...
export function ProductionStatusPanel({ projectId, view, onPrimaryAction }: Props): JSX.Element {
  const { t } = useTranslation()
  const previewUrl = view.preview
    ? buildNomiLocalAssetUrl(projectId, view.preview.thumbnailRelativePath)
    : null
  ...
  return (
    ...
    >
      ...
      <figure data-production-preview className={cn('m-0 overflow-hidden rounded-nomi border border-nomi-line-soft bg-nomi-ink-05')}>
        {previewUrl ? (
          <img
            src={previewUrl}
        ...
        <figcaption className={cn('flex items-center justify-between gap-2 px-2.5 py-2 text-caption')}>
          <span className={cn('font-medium text-nomi-ink-80')}>{t('generationCommon.production.runPanel.latestArtifact')}</span>
          <span className={cn('text-nomi-ink-40')}>
            {view.preview
              ? t(`generationCommon.production.artifactKind.${view.preview.kind}`)
