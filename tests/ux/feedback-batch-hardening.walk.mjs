...
// Visual and interaction journey.
{
  ...
  await shot(win, '01-dark-toolbar-clearance.png')

  const edgeTag = win.locator('.generation-canvas-v2__edge-tag-pill').filter({ hasText: '风格' }).first()
  await edgeTag.click()
  await win.waitForTimeout(250)
  await shot(win, '02-edge-mode-menu.png')
  ...
  await shot(win, '03-result-stack-actions.png')
  ...
  await win.waitForTimeout(500)
  await shot(win, '04-project-assets-all-results.png')
  ...
  await shot(win, '05-all-assets-delete-action.png')
  ...
  await shot(win, '06-light-assets-and-type.png')
  await app.close()
}
...
fs.writeFileSync(projectFile, '{bad json')
{
  ...
  await shot(win, '07-recovered-project.png')
